...
namespace flashgg {
    ...
    class ElectronScaleEGMTool: public BaseSystMethod<flashgg::Electron, int>
    {

    public:
        typedef StringCutObjectSelector<Electron, true> selector_type;

        ElectronScaleEGMTool( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer *gv );
        void applyCorrection( flashgg::Electron &y, int syst_shift ) override;
    ...
    ElectronScaleEGMTool::ElectronScaleEGMTool( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer *gv ) :
    ...
    void ElectronScaleEGMTool::eventInitialize( const edm::Event &iEvent, const edm::EventSetup & iSetup ) {
        run_number_ = iEvent.run();
    }
    
    std::string ElectronScaleEGMTool::shiftLabel( int syst_value ) const
    {
        std::string result;
        if( syst_value == 0 ) {
    ...
    void ElectronScaleEGMTool::applyCorrection( flashgg::Electron &y, int syst_shift )
    {
        if( overall_range_( y ) ) {
            ...
            if( debug_ ) {
                std::cout << "  " << shiftLabel( syst_shift ) << ": Electron has pt= " << y.pt() << " eta=" << y.eta()
...
DEFINE_EDM_PLUGIN( FlashggSystematicElectronMethodsFactory,
                   flashgg::ElectronScaleEGMTool,
                   "FlashggElectronScaleEGMTool" );
// Local Variables:
// mode:c++
