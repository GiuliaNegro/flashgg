...
namespace flashgg {
    ...
    class ElectronSmearStochasticEGMTool: public BaseSystMethod<flashgg::Electron, std::pair<int, int> >
    {
        
    public:
        typedef StringCutObjectSelector<Electron, true> selector_type;
        
        ElectronSmearStochasticEGMTool( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer *gv );
        void applyCorrection( flashgg::Electron &y, std::pair<int, int> syst_shift ) override;
    ...
    void ElectronSmearStochasticEGMTool::eventInitialize( const edm::Event &iEvent, const edm::EventSetup & iSetup ) {
        run_number_ = iEvent.run();
    }
    
    ElectronSmearStochasticEGMTool::ElectronSmearStochasticEGMTool( const edm::ParameterSet &conf, edm::ConsumesCollector && iC, const GlobalVariablesComputer *gv ) :
    ...
    std::string ElectronSmearStochasticEGMTool::shiftLabel( std::pair<int, int> syst_value ) const
    {
    ...
    void ElectronSmearStochasticEGMTool::applyCorrection( flashgg::Electron &y, std::pair<int, int> syst_shift )
    {
        if( overall_range_( y ) ) {
            ...
            if (!y.hasUserFloat(random_label_)) {
                throw cms::Exception("Missing embedded random number") << "Could not find key " << random_label_ << " for random numbers embedded in the Electron object, please make sure to read the appropriate version of MicroAOD and/or access the correct label and/or run the PerPhotonDiPhoton randomizer on-the-fly";
            }
            float rnd = y.userFloat(random_label_);
            float newe = y.energy() * (1. + rnd * sigma);
            if( debug_ ) {
                std::cout << "  " << shiftLabel( syst_shift ) << ": Electron has energy= " << y.energy() << " eta=" << y.eta()
...
DEFINE_EDM_PLUGIN( FlashggSystematicElectronMethodsFactory2D,
                   flashgg::ElectronSmearStochasticEGMTool,
                   "FlashggElectronSmearStochasticEGMTool" );
